...
class Reinforce(object):
    ...
    def __init__(self, model, lr):
        self.model = model
        self.model.compile(loss=losses.categorical_crossentropy, optimizer=keras.optimizers.Adam(learning_rate=0.001))
    ...
    def train(self, env, gamma=1.0):
        ...
        self.model.fit(np.array(states),-1*np.array(G))
    ...
    def generate_episode(self, env, render=False):
        ...
        state = env.reset()
        while not done:
            act_probs = self.model.predict(state)    # evaluate policy
            act = np.random.choice(1, p = act_probs)
    ...
    def test(self,env,gamma = 1):
        ...
        for i in range(20):
            ...
            while done == False:
                ...
                act_probs = self.model.predict(state).tolist()[0]
                act = np.random.choice(1, p = act_probs)
